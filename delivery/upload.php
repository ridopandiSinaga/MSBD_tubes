			...
			if($_FILES['file']['name']){
				...
				if($insert == true){
					if($filename[1] == 'csv'){
						...
						$cek =  fgetcsv($handle,0,','); //baca isi csv perbaris dengan koma
						if(!empty($cek[1])) { 
						$delimiter=",";
						}
						else { 
						...
						while (!feof($handle)) {
							...
							{ 
								...
								$sell     = number_format($tax_rate)/100;
								$sell_price = number_format($buy_price) * $sell;
								$sell_total = $sell_price + number_format($buy_price);
